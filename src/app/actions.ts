...
// Main streaming chat action
export async function streamingChat(input: { 
    history: CoreMessage[], 
        ...
        if (isSearch) {
            ...
            try {
                ...
                if (results.length > 0) {
                    ...
                    streamable.done(JSON.stringify(responsePayload));
                } else {
                    streamable.done("Sorry, I couldn't find any relevant websites for that search.");
                }
            } catch (error: any) {
                streamable.done(`Sorry, an error occurred during the search: ${error.message}`);
            }
            return;
        }
             ...
             try {
                ...
                if (video.id) {
                    ...
                    streamable.done(JSON.stringify(responsePayload));
                     ...
                     streamable.done("Sorry, I couldn't find a matching song on YouTube.");
                }
             } catch (error: any) {
                 streamable.done(`Sorry, an error occurred while searching YouTube: ${error.message}`);
             }
             return;
        }
