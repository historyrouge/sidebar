...
const GeneratePresentationInputSchema = zod_1.z.object({
    topic: zod_1.z.string().describe('The topic for the presentation.'),
    numSlides: zod_1.z.number().min(5).max(10).describe('The desired number of slides.'),
    style: zod_1.z.enum(['colorful', 'simple']).describe('The visual style preference for the presentation.'),
    colors: zod_1.z.string().optional().describe('Optional user-preferred colors (e.g., "blue and yellow").'),
});
const SlideSchema = zod_1.z.object({
    slideType: zod_1.z.enum(['title', 'overview', 'content', 'summary', 'closing']).describe("The type of the slide, which determines its layout."),
    title: zod_1.z.string().describe("The title of the slide."),
    content: zod_1.z.array(zod_1.z.string()).describe("An array of key points or content for the slide body, formatted as bullet points."),
    speakerNotes: zod_1.z.string().describe("Notes for the speaker for this slide, explaining what to say."),
    visualSuggestion: zod_1.z.string().optional().describe("A suggestion for a relevant visual element, like an icon or diagram description."),
});
const GeneratePresentationOutputSchema = zod_1.z.object({
    title: zod_1.z.string().describe('The main title of the overall presentation.'),
    colorTheme: zod_1.z.object({
        background: zod_1.z.string().describe("A hex code for the slide background color (e.g., #F4F4F8)."),
        primary: zod_1.z.string().describe("A hex code for primary text and headings (e.g., #0A0A0A)."),
        accent: zod_1.z.string().describe("A hex code for accents, like bullets or highlights (e.g., #607EA3)."),
    }).describe("A suggested color theme for the presentation."),
    slides: zod_1.z.array(SlideSchema).describe('An array of slides for the presentation.'),
});
